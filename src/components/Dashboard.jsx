...
function Dashboard({ analysis }) {
  ...
  const kpis = [
    { label: 'إجمالي الطلاب', value: summary.totalStudents, icon: Users, color: 'text-blue-400' },
    { label: 'نسبة النجاح', value: `${summary.passRate}%`, icon: TrendingUp, color: 'text-green-400' },
    { label: 'متوسط الدرجات', value: summary.averageGrade, icon: Award, color: 'text-yellow-400' },
    { label: 'طلاب في خطر', value: summary.atRiskCount, icon: AlertTriangle, color: 'text-red-400' },
  ]
  ...
  return (
    <div className="min-h-screen bg-slate-900 px-6 py-10">
        ...
        {kpis.map((kpi, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-slate-400 text-sm mt-3">{kpi.label}</p>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        ...
        >
          <h3 className="text-white font-semibold mb-4">توزيع مستوى الخطر</h3>
        ...
        <motion.div
          ...
          className="bg-slate-800 border border-slate-700 rounded-2xl p-6"
        >
          <h3 className="text-white font-semibold mb-4">متوسط الأداء حسب المكوّن</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={componentAverages}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis dataKey="clo" stroke="#94a3b8" />
              <YAxis stroke="#94a3b8" />
              <Tooltip
                contentStyle={{ backgroundColor: '#1e293b', border: 'none', borderRadius: 8, color: '#fff' }}
              />
      ...
      >
        <h3 className="text-white font-semibold mb-4">توصيات الذكاء الاصطناعي</h3>
        <div className="grid md:grid-cols-3 gap-4">
          {recommendations.map((rec) => (
            <div key={rec.id} className="bg-slate-800 border border-slate-700 rounded-2xl p-5">
              <h4 className="text-white font-medium mb-2">{rec.title}</h4>
              <p className="text-slate-400 text-sm mb-3">{rec.reason}</p>
              <div className="border-t border-slate-700 pt-3">
                <p className="text-blue-400 text-sm">{rec.action}</p>
              </div>
            </div>
          ))}
        </div>
